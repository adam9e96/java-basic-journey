 ...
 * 
 * @todo 성적범위 100점 이내만 받도록 수정하기
 */
public class Method06_ex {

	static final int MAX_STUDENT = 5; // 총인원
	...
	public static int getMenu(Scanner scanner) {
		int memu = -1;
		try {
			System.out.println(green + "┌───────[성적 관리 프로그램]─────────┐");
			...
			memu = scanner.nextInt();
		} catch (Exception e) {
//			System.out.println(e.toString());
			NoticeInputTypeMiss();
			scanner.nextLine(); // 잘못된 값 버퍼 비우기
		}
		return memu;
	}

	// 이름, 점수(국영수), 총점, 평균점수
	public static int createStudent(int studentCount, String[] name, int[][] scores, int[] totals,
			double[] totalsAverages, Scanner scanner) {
		try {
			String tempName = "";
			int kor, eng, math = 0;
			...
			// 여기까지 왔으면 문제없으니 넣기
			scores[studentCount][0] = kor;
			scores[studentCount][1] = eng;
			scores[studentCount][2] = math;
		} catch (Exception e) {
//			System.out.println(e.toString());
			System.out.println(red + "┌──────────────────────────────────────────┐");
...
//		System.out.println("평균 테스트:" + avg);
	...
	public static void getPrint(int studentCount, String[] name, int[][] scores, int[] totals, double[] totalsAverages,
			Scanner scanner) {
...
//			System.out.println(e.toString());
		...
		updateCalcSum(updateNum, studentCount, scores, totals); // sum 계산
		updateCalcAvg(updateNum, studentCount, totalsAverages, totals); // avg 계산
	}

	// sum 계산 함수
	public static void updateCalcSum(int updateNum, int studentCount, int[][] scores, int[] totals) {
		int sum = 0;
		for (int i = 0; i < scores[updateNum - 1].length; i++) {
			sum += scores[updateNum - 1][i];
		}
		totals[updateNum - 1] = sum;
	}

	/**
	 * 평균 계산 함수
	 */
	public static void updateCalcAvg(int updateNum, int studentCount, double[] totalsAverages, int[] totals) {
		totalsAverages[updateNum - 1] = totals[updateNum - 1] / 3.0;
	}
	...
	public static void searchStudent(int studentCount, String[] name, String searchName, int[][] scores, int[] totals,
...
//		String filePathSurface = "D:\\Project\\Study\\JavaBasicJourney\\src\\ch07_method\\sec02_ex\\data\\save.txt";
		...
		return new File(filePath); // File 객체 생성 및 반환
	}

	public static void writeFile(File file, int studentCount, String[] name, int[][] scores, int[] totals,
			double[] totalAverages) {
		try {
	...
	public static int readFile(File fileRead, int studentCount, String[] name, int[][] scores, int[] totals,
			double[] totalAverages) {
		try {
...
//                System.out.println(line); // 테스트 코드
			...
			System.out.println(e.toString());
			// 반환을 menu값 -1이나 0으로 해서 탈출하도록 수정하기
			isRunning = false;
